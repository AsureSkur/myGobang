...
int alphabetaSearch(int depth, int alpha, int beta, bool role, int color, struct Board& board, CNNNetwork& cnn, int& winflag)
{
	...
	for (int i = 0; i < moves.size(); i++)
	{
		...
		int y = moves[i].y;
		board.grid[x][y] = (color == 1) ? BLACK : WHITE;
		moves[i].val = alphabetaSearch(depth - 1, alpha, beta, !role, -color, board, cnn, winflag);
		value = stateEvaluate(board, maxColor, cnn, winflag);
		moves[i].val = winflag;
		board.grid[x][y] = 0;
		winflag = 0;
	...
	if (cnt_win > cnt_lose)
	{
		cnn.forward_backward(board, 1, 0);
	}
	else if(cnt_win<cnt_lose)
	{
		cnn.forward_backward(board, 2, 0);
	}
	if (role)
	{
		return alpha;
