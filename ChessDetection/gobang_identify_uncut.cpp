...
void HoughCheckChess(const char* imgname, std::vector<cv::Vec3f>& circles, struct HoughVal value)
{
	...
	int maxRadius = (srcImage.rows + srcImage.cols) / (value.step + 14) / 4 + 10;
...
void posCheckChess(cv::InputArray src, std::vector<cv::Vec3f>& circles,double step,double rest, int(*chessboard)[15])
{
	int rows = 15;
	int x, y;
	double x0, y0;
	...
	{
		x0 = circles[i][0] * (step + 14) / src.rows() - step * rest;
		y0 = circles[i][1] * (step + 14) / src.cols() - step * rest;
		x = x0 / 1;
		y = y0 / 1;
		double cmp[4];
...
int main(int argc, const char* argv[])
{
	...
	while (1)
	{
		...
		for (int i = 1; i <= TESTNUM; i++)
		{
			...
			posCheckChess(imread(filepath), circles[i - 1],val.step,val.rest, chessboard);
